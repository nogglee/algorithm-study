...
공식 이해에 도움받은[유튜브 링크](https://www.youtube.com/watch?v=4HVFHhvvpOk)  
...
공식 이해에 도움받은 [유튜브 링크](youtube.com/watch?v=C8JlalZR014)  

1~4까지 모든 항의 제곱의 합을 구하려면 아래와 같은 식을 세울 수 있다.  
$\boxed {S(5) = 1^2 + 2^2 + 3^2 + 4^2 + 5^2}$  
...
$\boxed{ S(n) = \frac{1}{3}n^3 + \frac{1}{2}n^2 + \frac{1}{6}n }$  

계산하기 쉽게 통분해서 정리해보자.  
$\boxed{ S(n) = \frac{2n^3 + 3n^2 + n}{6} }$  
...
$\boxed{ 2 = 2×1, 4 = 2×2, 6 = 2×3, 8 = 2×4, 10 = 2×5 }$  

위 규칙을 2k라는 수식으로 표현 할 수 있다.  
n의 최대값인 10을 수식으로 변환하면, k의 값은 5로 `2k = n` 으로 표현 할 수 있다.  
짝수 n의 개수 또한 $\frac{n}{2}$ 로 계산 할 수 있으며, 결국 k의 최대값은 $\frac{n}{2}$ 가 된다.  


$\boxed{ \frac{m(m+1)(2m+1)}{6} }$  
자연수 거듭 제곱의 합 공식을 위와 같이 세웠을 때, 최댓값을 활용해 연속된 수들의 제곱의 합을 구할 수 있으니  
이 전에 구한 k의 최댓값인 ' $\frac{n}{2}$ ' 를 m에 대입한다.  

여기서 또 하나의 과정을 추가해주어야한다.  
먼저 n이하의 모든 짝수 제곱의 합을 구하는 식은 아래와 같다.  
$2^2 + 4^2 + 6^2 + ... + n^2$  

현재 m에는 $\frac{n}{2}$ 가 적용되어 있으므로, 아래와 같은 식이 세워져있다.  
$1^2 + 2^2 + 3^2 + 4^2 + 5^2$  
...
**자연수 거듭 제곱의 합 공식**  
$\boxed{ \sum_{k=1}^{m} k^2 = \frac{m(m+1)(2m+1)}{6} }$  


k는 1부터 시작되며, m은 마지막 k 값이된다.  
'짝수인 자연수'라는 조건이 붙으면 k의 값은 1부터 $\frac{n}{2}$ 까지 이므로,  
1. m은 $\frac{n}{2}$ 로 대입되어야한다.  
2. 짝수를 2k로, 짝수의 제곱을 $(2k)^2$ 로 표현해야한다.  

조건을 적용한 공식을 시그마로 표현하면 아래와 같다.  


1. 짝수인 자연수 거듭 제곱의 합 공식  
$\boxed{ \sum_{k=1}^{m} 4k^2 = \frac{m(m+1)(2m+1)}{6} }$

2. 4는 상수이니 밖으로 꺼내보자.  
$\boxed{ 4 \sum_{k=1}^{m} k^2 = \frac{m(m+1)(2m+1)}{6} }$

3. m에 $\frac{n}{2}$를 대입해보자.  
$\boxed{ \frac{4}{6} \times \frac{n}{2} \times \left(\frac{n}{2}+1\right) \times (n+1) }$  

4. 위 식을 좀 더 정돈해보자.  
$\boxed{ \frac{2}{3} \times \frac{n}{2} \times \left(\frac{n}{2}+1\right) \times (n+1) }$  
